            ...
            if method == "Ordinary Least Squares":              
                ...
                for r in range(self.iv_table.rowCount()):
                    ...
                    ax1.plot(seg.iloc[:, 0], fit_I, color=colors[seg_id % 4], lw=6,
                        label=f"Seg {seg_id} dI/dV = {res.slope * self.scale_i / self.scale_v:.4g} {self.unit_i.currentText()}/{self.unit_v.currentText()}",
                        alpha = 0.6)
                    results.append([
                        f"Seg {seg_id}", 
                        f"({v_start}, {v_end})", 
                ...
                for r in range(self.pv_table.rowCount()):
                    try:
                        ...
                        ax2.plot(seg.iloc[:, 0], fit_P, color=colors[seg_id % 4], lw=6,
                                label=f"Seg {seg_id} dP/dV = {res.slope * self.scale_p / self.scale_v:.4g} {self.unit_p.currentText()}/{self.unit_v.currentText()}",
                                alpha=0.6)
                        results.append([
                            f"Seg {seg_id}", 
                            f"({v_start}, {v_end})", 
                ...
                QMessageBox.warning(self, "Error", "Least Absolute Residual fitting is not implemented yet.")
                return
