...
class PlotApp(QMainWindow):
    def __init__(self):
        ...
        load_button = QPushButton("Choose a file (.dat)")
        load_button.clicked.connect(self.load_data)

        plot_button = QPushButton("Plot")
        plot_button.setFixedSize(100, 23)
        plot_button.clicked.connect(self.plot_data)

        topleft_layout.addWidget(load_button)
        topleft_layout.addWidget(plot_button)
        ...
        fit_btn.setFixedSize(100, 23)
        fit_btn.clicked.connect(self.fit_data)
        control_layout.addWidget(fit_btn, alignment=Qt.AlignRight)
        center_layout.addLayout(control_layout, 2)
        ...
        tif_cal_button.setFixedSize(100, 23)
        tif_cal_button.clicked.connect(self.tif_cal)

        save_button = QPushButton("Save")
        save_button.setFixedSize(100, 23)
        save_button.clicked.connect(self.save)

        tif_left_layout.addWidget(tif_box)
        tif_left_layout.addWidget(tif_cal_button, alignment=Qt.AlignRight)

        tif_right_layout.addWidget(self.result_tif)
        tif_right_layout.addWidget(save_button, alignment=Qt.AlignRight)

        tif_layout.addLayout(tif_left_layout, 3)
        tif_layout.addLayout(tif_right_layout, 5)
    ...
    def save(self):
        ...
        if self.figure is not None:
            file_name, _ = QFileDialog.getSaveFileName(
                self, "Save Plot Image", f"plot_{datetime.datetime.now():%Y%m%d_%H%M%S}.png",
        ...
        if self.result_table.rowCount() > 0:
            ...
            file_name, _ = QFileDialog.getSaveFileName(
                self, "Save Fit Results CSV", f"fit_results_{datetime.datetime.now():%Y%m%d_%H%M%S}.csv",
                "CSV Files (*.csv);;All Files (*)"
            )
            if file_name:
                df.to_csv(file_name, index=False)

        # Save Tif calculation table
        if self.result_tif.rowCount() > 0:
            tif_results = []
            row_labels = [self.result_tif.verticalHeaderItem(i).text() for i in range(self.result_tif.rowCount())]
            for row in range(self.result_tif.rowCount()):
                value_item = self.result_tif.item(row, 0)
                unit_item = self.result_tif.item(row, 1)
                value_text = value_item.text() if value_item else ""
                unit_text = unit_item.text() if unit_item else ""
                # Check for empty or NaN
                if value_text.strip() == "" or value_text.strip().lower() == "nan":
                    continue
                tif_results.append({
                    "Parameter": row_labels[row],
                    "Value": value_text,
                    "Unit": unit_text
                })
            if tif_results:
                df_tif = pd.DataFrame(tif_results)
                file_name, _ = QFileDialog.getSaveFileName(
                    self, "Save Tif Calculation CSV", f"tif_calc_{datetime.datetime.now():%Y%m%d_%H%M%S}.csv",
                    "CSV Files (*.csv);;All Files (*)"
                )
                if file_name:
                    df_tif.to_csv(file_name, index=False)
