...
class SpectrumPlot(QWidget):
    def __init__(self):
        ...
        # 按鈕
        self.hot_button = QPushButton("Hot Data")
        self.cold_button = QPushButton("Cold Data")
        ...
        #2nd line button
        self.home_button = QPushButton("init Load")
        self.hot_label_button = QPushButton("move to Hot Load")
        self.cold_label_button = QPushButton("move to Cold Load")

        # only home & cold load working at init. ：
        self.hot_label_button.setEnabled(False)
        self.cold_label_button.setEnabled(True)

        self.home_button.clicked.connect(self.handle_home)
        self.hot_label_button.clicked.connect(self.handle_hot)
        self.cold_label_button.clicked.connect(self.handle_cold)


        b2_layout = QHBoxLayout()
        b2_layout.addWidget(self.home_button)
        b2_layout.addWidget(self.hot_label_button)
        b2_layout.addWidget(self.cold_label_button)
    ...
    def handle_cold(self):
        try:
            self.hot_label_button.setEnabled(True)
            self.cold_label_button.setEnabled(False)
